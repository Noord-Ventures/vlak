...
const budgets = [
  // The 146-component catalog adds civic, science and creative workflows.
  // Measured aggregate costs are 25.6 KB CSS, 162.4 KB JS and 80.1 KB CLI.
  // Keep atomic CSS and existing control caps; bound every new leaf separately.
  ["@noorddev/vlak css/vlak.css", "packages/core/css/vlak.css", 27 * 1024],
  ["@noorddev/vlak-react dist/vlak-react.css", "packages/react/dist/vlak-react.css", 16 * 1024],
  ["@noorddev/vlak-react dist/**/*.js (every component, bundled)", ["packages/react/dist", /\.js$/], 166 * 1024],
  ...
  ...["activity-rings", "identity-document", "tax-summary", "benefit-program", "application-status", "evidence-checklist", "measurement-value", "quantity-field", "experiment-run", "audio-meter", "channel-strip", "parameter-knob", "timecode-field", "clip-timeline", "render-queue", "layer-stack", "color-inspector", "spacing-control"].map(name =>
    [`@noorddev/vlak-react components/${name}.js`, `packages/react/dist/components/${name}.js`, 3 * 1024]),
  ...["well-plate", "spectrum-plot"].map(name =>
    [`@noorddev/vlak-react components/${name}.js`, `packages/react/dist/components/${name}.js`, 4 * 1024]),
  ["@noorddev/vlak-cli dist/index.js (bundles the typed registry for list/search)", "packages/cli/dist/index.js", 84 * 1024],
];
