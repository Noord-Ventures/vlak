...
export function createDriveScene(host: HTMLDivElement, initial: DriveSceneState, failed: () => void) {
  ...
  const model = createDriveModel();scene.add(model.root);
  ...
  for(const z of [-.77,.77]) {
    ...
    lamp.position.set(-2.49,1.06,z); lamp.target.position.set(-8.4,-.15,z*.8);
  ...
  const orbit=new THREE.Spherical(1,Math.PI/2,0),desiredOrbit=new THREE.Spherical();
  const palette=()=>{
  ...
  const render=(now:number)=>{
    ...
    desiredEye.set(journey?10:energy?-4.8:0,journey?5.2:energy?5.8:0,journey?2.4:energy?7.6:10).normalize();
    ...
    // Orbit around the same object instead of cutting a chord through the camera
    // directions. Keeping the live angles makes a change of view reversible mid-orbit.
    desiredOrbit.setFromVector3(desiredEye);
    const turn=Math.atan2(Math.sin(desiredOrbit.theta-orbit.theta),Math.cos(desiredOrbit.theta-orbit.theta));
    orbit.theta+=turn*rate;orbit.phi=THREE.MathUtils.lerp(orbit.phi,desiredOrbit.phi,rate);
    eye.setFromSpherical(orbit);target.lerp(desiredTarget,rate);
    // Equal proportional zoom steps preserve the schematic's apparent scale as
    // perspective opens up, including when a transition starts from an in-between view.
    span=snap?nextSpan:Math.exp(THREE.MathUtils.lerp(Math.log(span),Math.log(nextSpan),rate));
    // Finish the subpixel remainder so the settled schematic is exactly side-on.
    const cameraMoving=eye.distanceTo(desiredEye)>.0001||target.distanceTo(desiredTarget)>.0001||Math.abs(span-nextSpan)>.001||Math.abs(fov-nextFov)>.001;
    if(!cameraMoving){orbit.copy(desiredOrbit);eye.copy(desiredEye);target.copy(desiredTarget);span=nextSpan;fov=nextFov;}
    ...
    floor.visible=world.root.visible||headlights.visible;
    groundMaterial.opacity=.6*(1-worldOpacity)*vehicleOpacity;groundLine.visible=groundMaterial.opacity>.002;
    ...
    const transitioning=cameraMoving||Math.abs(worldOpacity-(journey?1:0))>.002||Math.abs(lightPower-(state.lights&&!energy?1:0))>.002||Math.abs(bodyLift-(energy?.30:0))>.002||Math.abs(coverLift-(energy?.90:0))>.002||Math.abs(vehicleOpacity-(energy?0:1))>.002;
