...
import { CalendarDateField } from "./date-field";
...
export function EventEditor({ editor, calendars, weekStart, onClose, onSave, onDelete }: {
  editor: EditorState;
  ...
  return <Dialog open onClose={close} className="cal-dialog cal-editor">
    <form onSubmit={submit}>
      ...
      <div className="cal-dialog-content">
        ...
        <div className="cal-field-pair"><CalendarDateField weekStart={weekStart} label="Starts" required type={draft.allDay ? "date" : "datetime-local"} min={draft.allDay ? "1900-01-01" : "1900-01-01T00:00"} max={draft.allDay ? "2200-12-30" : "2200-12-30T23:59"} value={draft.allDay ? draft.start.slice(0, 10) : draft.start} onValueChange={value => changeStart(draft.allDay ? `${value}T00:00` : value)} /><CalendarDateField weekStart={weekStart} label={draft.allDay ? "Last day" : "Ends"} required type={draft.allDay ? "date" : "datetime-local"} min={draft.allDay ? draft.start.slice(0, 10) : draft.start} max={draft.allDay ? "2200-12-30" : "2200-12-31T23:59"} value={draft.allDay ? addDays(draft.end.slice(0, 10), -1) : draft.end} onValueChange={value => { if (draft.allDay && (!isDate(value) || value >= "2200-12-31")) return; update({ end: draft.allDay ? `${addDays(value, 1)}T00:00` : value }); }} /></div>
        {(!isSeries || scope === "series") && <>
          ...
          {recurrence && <div className="cal-recurrence-fields">
            ...
            <div className="cal-field-pair"><NativeSelect label="Ends" value={recurrence.until ? "until" : recurrence.count ? "count" : "never"} onChange={e => update({ recurrence: { ...recurrence, until: e.target.value === "until" ? addDays(validStartDate > "2200-11-30" ? "2200-11-30" : validStartDate, 30) : undefined, count: e.target.value === "count" ? 10 : undefined } })}><option value="never">Never</option><option value="until">On a date</option><option value="count">After a number</option></NativeSelect>{recurrence.until && <CalendarDateField weekStart={weekStart} label="Repeat until" type="date" required min={draft.start.slice(0, 10)} max="2200-12-31" value={recurrence.until} onValueChange={value => update({ recurrence: { ...recurrence, until: value } })} />}{recurrence.count !== undefined && <Input label="Occurrences" type="number" required min={1} max={10000} value={recurrence.count} onChange={e => update({ recurrence: { ...recurrence, count: Number(e.target.value) } })} />}</div>
