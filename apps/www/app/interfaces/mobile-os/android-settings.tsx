...
const categories: { id: string; icon: IconName; subtitle: string }[] = [
  ...
  { id: "about", icon: "info", subtitle: "Avery’s Pixel" },
];
...
export function AndroidSettings(p: SettingsProps) {
  if (!p.screen) return <><input type="search" className="mo-android-settings-search" aria-label="Search settings" placeholder="Search settings" value={p.search} onChange={event => p.onSearch(event.target.value)} /><div className="mo-android-settings-list">{categories.filter(row => `${androidSettingsTitles[row.id]} ${row.subtitle}`.toLowerCase().includes(p.search.toLowerCase())).map(row => <Button className="mo-android-setting-row" variant="ghost" key={row.id} onClick={() => p.navigate(row.id)}><>{row.id === "battery" ? <PlatformGlyph name="battery" /> : <Icon name={row.icon} size={24} />}</><span><strong>{androidSettingsTitles[row.id]}</strong><small>{row.subtitle}</small></span></Button>)}</div></>;
  ...
  return <div className="mo-settings-section"><div className="mo-data-row"><span>Device name</span><strong>Avery’s Pixel</strong></div><div className="mo-data-row"><span>Android version</span><strong>15 reference</strong></div><div className="mo-data-row"><span>Storage</span><span>This browser tab</span></div><p className="mo-caption">A fictional local device. No Google Account is connected.</p></div>;
}
