...
export async function checkCalendar({ page, base, fail }) {
  ...
  try {
    ...
    await createDialog.getByLabel("Starts", { exact: true }).fill(`${date}T11:00`);
    await createDialog.locator('input[type="datetime-local"]').nth(1).fill(`${date}T12:30`);
    ...
    const occurrenceStart = await editDialog.getByLabel("Starts", { exact: true }).inputValue();
    ...
    assert.equal(await editDialog.getByLabel("Starts", { exact: true }).inputValue(), original.start, "Series editing restores the schedule's original start");
    ...
    await createDialog.getByLabel("Last day", { exact: true }).fill(tomorrow);
    await saveEvent(createDialog);
