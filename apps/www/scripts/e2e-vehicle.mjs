...
import { webglArgs, distance, inspectModel, checkModelControls } from "./e2e-render-controls.mjs";

const base = (process.env.SITE_URL ?? "http://localhost:3100").replace(/\/$/, "");
const artifacts = await mkdtemp(join(tmpdir(), "vlak-object-"));
...
try {
  ...
  const focused = await inspectModel(page);
  await page.waitForTimeout(250);
  assert(distance(focused.position, (await inspectModel(page)).position) < .00001, "Focus must pause the turntable");
  ...
  assert(distance(home.position, (await inspectModel(page)).position) < .00001, "Paused camera must stay still");
  await page.setViewportSize({ width: 390, height: 844 });
  await page.locator(".rw").scrollIntoViewIfNeeded();
  await page.waitForFunction(() => { const canvas = document.querySelector(".rw-object-canvas"); return canvas?.dataset.camera === canvas?.dataset.home && Number(canvas?.dataset.viewportWidth) <= 390; });
  const phone = await inspectModel(page);
  assert(phone.homeFit.split(",").map(Number).every(value => Math.abs(value) <= .96), "Portrait camera must fit every actual object vertex, including tall assets already limited by height");
  ...
  await unavailable.route("**/braun-t3-monochrome.glb", route => route.abort("internetdisconnected"));
  await unavailable.goto(`${base}/interfaces/render/`, { waitUntil: "domcontentloaded" });
  await unavailable.locator('.rw-object-viewport[data-viewer-status="error"]').waitFor({ timeout: 15000 });
  assert.match(await unavailable.locator(".rw-object-status").innerText(), /connection and WebGL/);
  ...
  await unavailable.unroute("**/braun-t3-monochrome.glb");
  await unavailable.getByRole("button", { name: "Retry viewer", exact: true }).click();
  await unavailable.waitForFunction(() => document.querySelector(".rw")?.dataset.viewerStatus === "ready" && Number(document.querySelector(".rw-object-canvas")?.dataset.renderedTriangles) >= 32027);
  assert(await unavailable.getByRole("button", { name: "Show mesh", exact: true }).isEnabled());
  assert.equal(await unavailable.locator(".rw-object-canvas").count(), 1);
  ...
  if (await unavailable.evaluate(() => !!window.renderRoot)) {
    const canvas = await unavailable.locator(".rw-object-canvas").elementHandle();
  ...
  console.log("Object integration passed: real mesh, paint and camera; turntable, focus pause, responsive fit, reduced motion, asset failure, retry and renderer cleanup");
