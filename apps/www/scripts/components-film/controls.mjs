...
export function createControls(THREE, kit) {
  ...
  const underside = frame(6.28,3.36,.14,m.silver,1.68,.07);
  const thumb = new THREE.Group();
  const thumbFoot = box(2.47,2.47,.1,m.rubber,1.235);
  const thumbBody = box(2.47,2.47,.31,m.porcelain,1.235);
  const thumbRim = frame(2.45,2.45,.042,m.silver,1.225,.024);
  const thumbTop = box(2.36,2.36,.045,m.paper,1.18);
  thumb.add(thumbFoot,thumbBody,thumbRim,thumbTop);
  toggle.add(underside,rail,outline,inner,thumb);
  ...
  for(let row=0;row<2;row++) {
    ...
    const fill = box(11.6,.068,.09,m.ink,.015);fill.position.z=.025;
    const knob = new THREE.Group();
    const ring=box(.49,.49,.17,m.gray,.245);
    const cap=box(.39,.39,.18,m.paper,.195);cap.position.z=.07;
    knob.add(ring,cap);knob.position.z=.15;
  ...
  function update(t) {
    ...
    toggle.rotation.set(mix(-.12,0,pull),mix(-.1,0,pull),-.035*(1-pull));
    rail.position.set(-open*.12,0,0);
    underside.position.set(-open*.43,-open*.12,-.24-open*.8);
    outline.position.set(open*.3,open*.08,.17+open*.63);
    inner.position.z=.18+open*.15;
    const on=clamp(spring((t-2.6)*1.6));
    inner.material.opacity=on;
    thumb.position.set(mix(-1.455,1.455,on),0,.39+open*1.65);
    thumb.rotation.z=pulse(t,2.6)*.1;
    thumbFoot.position.z=-.22-open*.25;
    thumbBody.position.z=0;
    thumbRim.position.set(open*.1,0,.16+open*.25);
    thumbTop.position.set(open*.21,open*.08,.18+open*.48);
    ...
    rangeRows.forEach(({fill,knob,holder},index)=>{
      ...
      knob.position.z=.15+Math.abs(pulse(t,4.1+index*.25))*.12;
