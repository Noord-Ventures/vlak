...
test("project lifecycle preserves work through imports, conflicts, history, recovery, and deletion", async () => {
  ...
  try {
    ...
    assert.equal(recovery.projects.length, 2);
    ...
    await secondRows.last().getByRole("button", { name: "Remove" }).click();
    ...
    await secondTools.getByRole("button", { name: "Recent projects" }).click();
    assert.equal(await second.locator("dialog[open] .project-tools-list").first().locator("li").count(), 1);
