...
export function InterfaceStart({ source, slug, title, starterDownload }: { source: string; slug: string; title: string; starterDownload?: string }) {
  ...
  return <aside className="if-duo-start" id={slug === "ios" ? "duo-start" : "interface-start"} aria-label={`Start with ${title}`}>
    <div className="if-duo-start-copy">
      ...
      <span>{starterDownload ? "Download a working Vite and React project and change it." : "Use the study source and Vlak components in your project."}</span>
      <code>{install}</code>
    ...
    <div className="if-duo-start-actions">
      <a className="rs-btn-primary if-build-link" href={starterDownload ?? "/interfaces/"} download={!!starterDownload} data-vlak-starter={starterDownload ? slug : undefined}>{starterDownload ? "Download starter" : "Browse interfaces"} <span aria-hidden="true">{starterDownload ? "↓" : "→"}</span></a>
...
export function DuoStart({ source }: { source: string }) {
  return <InterfaceStart source={source} slug="ios" title="iPhone Duo" starterDownload="/starter/ios.zip" />;
}
