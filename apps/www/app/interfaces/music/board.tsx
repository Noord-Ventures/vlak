...
export function MusicBoard() {
  ...
  return <div className="mu-studio">
    <div className="mu-frame" data-view={view}>
      ...
      <nav className="mu-mobile-nav" aria-label="Music workspace"><ToggleGroup value={view} onValueChange={setView} options={[{ value: "session", label: "Session" }, { value: "clip", label: "Clip" }, { value: "mixer", label: "Mixer" }]} /></nav>
      <div className="mu-workspace">
        <section className="mu-session" aria-label="Session clips">
          ...
          <div className="mu-matrix-viewport" role="region" aria-label="Clip launcher, scroll to all tracks" tabIndex={0}>
            <div className="mu-matrix">
              ...
              {session.tracks.map((item, index) => <Button key={item.id} variant="ghost" className="mu-track-heading" aria-pressed={selected.track === index} onClick={() => { setSelected({ track: index, clip: item.active }); setView("mixer"); }}><span className="mu-track-index">{String(index + 1).padStart(2, "0")}</span><span>{item.name}</span></Button>)}
              {scenes.map((scene, sceneIndex) => <div className="mu-scene-row" key={scene}>
                ...
                {session.tracks.map((item, trackIndex) => <Button key={item.id} variant={item.active === sceneIndex ? "primary" : "ghost"} className="mu-clip" aria-pressed={item.active === sceneIndex} aria-label={`Launch ${item.clips[sceneIndex]!.name}, ${item.name}`} onClick={() => { updateTrack(trackIndex, value => ({ ...value, active: sceneIndex })); setSelected({ track: trackIndex, clip: sceneIndex }); setView("clip"); setNotice(`${item.clips[sceneIndex]!.name} selected for ${item.name}.`); }}><span><Icon name={item.active === sceneIndex && playing ? "activity" : "play"} size={12} />{item.clips[sceneIndex]!.name}</span><span className="mu-mini-pattern" aria-hidden="true">{item.clips[sceneIndex]!.steps.map((active, step) => <i key={`step-${step}`} data-active={active} />)}</span></Button>)}
              </div>)}
            </div>
          </div>
        ...
        <section className="mu-editor" aria-label="Clip editor">
          <div className="mu-panel-heading"><div><span className="mu-eyebrow">{track.name} / {scenes[selected.clip]}</span><h2>{clip.name}</h2></div><span>{clip.steps.filter(Boolean).length} notes</span></div>
          ...
          <div className="mu-pattern-viewport" role="region" aria-label="Sixteen-step pattern" tabIndex={0}>
            ...
            }}><span>{String(step + 1).padStart(2, "0")}</span><i aria-hidden="true" /></Button>)}</div>
          </div>
        ...
        <aside className="mu-mixer" aria-label="Instrument and mixer">
          <div className="mu-panel-heading"><div><span className="mu-eyebrow">Instrument</span><h2>{track.name}</h2></div><span>{String(selected.track + 1).padStart(2, "0")}</span></div>
          ...
          <AudioMeter label="Channel levels" min={-60} max={0} channels={session.tracks.map((item, index) => ({ id: item.id, label: item.name, level: playing ? playhead.levels[index]! : -Infinity }))} />
        </aside>
      </div>
