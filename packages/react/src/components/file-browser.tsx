...
export const FileBrowser = React.forwardRef<HTMLDivElement, FileBrowserProps>(function FileBrowser({ entries, label = "Files", value, defaultValue, onValueChange, folder, defaultFolder = null, onFolderChange, onOpen, className, style, ...props }, ref) {
  ...
  return <div ref={ref} role="region" aria-label={label} {...props} className={root.className} style={{ ...root.style, ...style }}>
    <div {...toolbar}><nav aria-label="Folder breadcrumbs"><ol {...pathStyle}><li><Button {...action} variant="ghost" aria-current={path.length === 0 ? "location" : undefined} onClick={() => navigate(null)}>{label}</Button></li>{path.map((entry, index) => <li key={entry.id}><Button {...action} variant="ghost" aria-current={index === path.length - 1 ? "location" : undefined} onClick={() => navigate(entry.id)}><Icon name="chevron-right" size={12} />{entry.name}</Button></li>)}</ol></nav></div>
    <div {...body}>
      <aside {...tree} aria-label="Folder navigation"><TreeView nodes={[{ id: rootId, label, children: folderNodes(entries) }]} label="Folders" value={currentFolder?.id ?? rootId} expanded={expandedIds} onExpandedChange={setExpandedIds} onValueChange={id => navigate(id === rootId ? null : id)} /></aside>
      <div {...content}>
