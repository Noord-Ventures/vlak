...
import { Attachment, Attachments, Button, MessageComposer, Toggle, useFileAttachments } from "@noorddev/vlak-react";
...
export function Use() {
  ...
  return <UseField name="message-composer"><UseType>Continue the conversation</UseType><UseBody><UseStack>
    <MessageComposer compact maxRows={6} sendOnEnter label="Message" placeholder="Ask about your files…" allowAttachments allowScreenshot
      ...
      tools={<Toggle variant="subtle" pressed={includeSources} onPressedChange={setIncludeSources}>Include sources</Toggle>}
    />
    <div className="rs-use-row"><Button variant="subtle" disabled={files.length > 2} onClick={() => setFiles((current) => [...current, new File(["Review the owner, source, and next decision."], "project-brief.txt", { type: "text/plain" }), new File([sketch], "concept-sketch.svg", { type: "image/svg+xml" })])}>Add example files</Button></div>
