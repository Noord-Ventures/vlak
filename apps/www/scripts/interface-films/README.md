...
Replace `press` with `line`, `wall`, `night`, `evening`, `room`, `graphics`, `render`, `drive`, `orbit`, `frontier`, `platforms`, or `robotics`. The existing Agents renderer remains `apps/www/scripts/render-agents-film.mjs --sound`.
...
Native input setters and keyboard events preserve the board's controlled state. After assets load, fixture timers and `Date.now()` run on film time, where the current board uses them. Character entry is deterministic. Assertions inspect the actual resulting DOM. Rewinding remounts the original fixture and replays actions. CSS animations use film time. The Three.js vehicle, robot and Athena scenes retain their native renderers. Fleet management uses Mapbox when a public token is supplied at build time, and otherwise shows the explicitly labeled local schematic. The current film configurations exercise the rebuilt boards; existing exported masters must be rendered again to show those changes.
...
The renderer loads the compiled React/core component CSS, original site reset, shared interface CSS and each board's own scene CSS. All original image assets are decoded before capture. A browser-only `next/dynamic` adapter imports the same original components. The 3D workspace waits for the locally hosted licensed vehicle geometry and fails if it is unavailable. The EV, robotics and Athena scenes likewise use their actual Three.js implementations. Vehicle and Athena creator credits appear in the study page below Components used, rather than inside the interface. Original local fixtures remain local demonstrations; no chat, delivery, agent or vehicle backend is contacted.
