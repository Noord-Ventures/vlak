...
export const aiResponse: VlakComponent[] = [
  {
    name: "response",
    title: "Response",
    description: "Presents user messages in right-aligned soft bubbles and assistant responses on the left, with speaker identity, status, and an action slot.",
    category: "ai",
    classes: ["rs-response", "rs-response-user", "rs-response-header", "rs-response-author", "rs-response-status", "rs-response-content", "rs-response-error", "rs-response-actions", "rs-response-copy", "rs-response-copy-status"],
    css: ["components/response.css"],
    react: "components/response.tsx",
    registryDependencies: [],
    snippet: '<article class="rs-response" aria-labelledby="response-author"><div class="rs-response-header"><span class="rs-response-author" id="response-author">Assistant</span><span class="rs-response-status" role="status" aria-live="polite" aria-atomic="true">Complete</span></div><div class="rs-response-content"><p>The lease renews on 1 October.</p></div></article>',
    example: "import { Response, ResponseActions } from \"@noorddev/vlak-react\";\n\nexport function LeaseExchange() {\n  return <>\n    <Response from=\"user\">When does the lease renew?</Response>\n    <Response from=\"assistant\" status=\"complete\" actions={<ResponseActions text=\"The lease renews on 1 October.\" />}>\n      <p>The lease renews on <strong>1 October</strong>.</p>\n    </Response>\n  </>;\n}",
    usage: {
      use: [
