...
export const inspectVehicle = async page => page.locator(".rw-vehicle-canvas").evaluate(canvas => ({
  ...canvas.dataset,
  position: canvas.dataset.camera.split(",").map(Number),
  target: canvas.dataset.target.split(",").map(Number),
}));
export async function openVehicle(page, base) {
  await page.goto(`${base.replace(/\/$/, "")}/interfaces/render/`, { waitUntil: "domcontentloaded" });
  await page.locator(".rw").scrollIntoViewIfNeeded();
  await page.waitForFunction(() => document.querySelector(".rw-vehicle-canvas")?.dataset.camera && document.querySelector(".rw")?.dataset.viewerStatus === "ready", undefined, { timeout: 45000 });
}
export async function checkVehicleControls({ page, base, artifacts, contextLoss = false }) {
  await openVehicle(page, base);
  const root = page.locator(".rw");
  const canvas = root.locator(".rw-vehicle-canvas");
  const initial = await inspectVehicle(page);
  assert.equal(initial.renderer, "three");
  assert.equal(initial.modelUrl, "/interfaces/concepts/evoque-monochrome.glb");
  assert.equal(Number(initial.triangles), 204453, "The full licensed geometry must be loaded");
  assert.equal(Number(initial.vertices), 147374);
  assert.equal(Number(initial.meshes), 66);
  assert.equal(Number(initial.materials), 21);
  assert.equal(initial.surfaceColor, initial.paper, "Line surfaces must match the composited viewport paper");
  assert(Number(initial.renderedTriangles) >= 204453, "The renderer must actually draw the vehicle geometry");
  ...
  await page.waitForFunction(() => document.querySelector(".rw-vehicle-canvas")?.dataset.lineInk === (document.documentElement.dataset.theme === "dark" ? "e0e0e0" : "252525"));
  ...
  await page.waitForFunction(() => Number(document.querySelector(".rw-vehicle-canvas")?.dataset.renderedLines) > 600000);
  assert(Number((await inspectVehicle(page)).wireframeMaterials) >= 5);
  ...
  await root.getByRole("button", { name: "Show mesh", exact: true }).press("Enter");
  await page.waitForFunction(() => document.querySelector(".rw-vehicle-canvas")?.dataset.wireframeMaterials === "0");
  ...
  const orbit = await inspectVehicle(page);
  await page.keyboard.press("+");
  await page.waitForFunction(before => {
    const element = document.querySelector(".rw-vehicle-canvas");
  ...
  assert(distance(home.position, (await inspectVehicle(page)).position) > .1, "Pointer drag must orbit the actual camera");
  await root.getByRole("button", { name: "Reset camera", exact: true }).click();
  await page.waitForFunction(() => { const canvas = document.querySelector(".rw-vehicle-canvas"); return canvas?.dataset.camera === canvas?.dataset.home; });
  ...
  const still = await inspectVehicle(page);
  await page.mouse.move(0, 0);
  await page.waitForTimeout(250);
  assert(distance(still.position, (await inspectVehicle(page)).position) < .00001, "Reduced motion must keep the camera still");
  if (mobile) {
    await nav.getByRole("button", { name: "Inspector", exact: true }).press("Enter");
    await page.waitForFunction(() => document.activeElement === document.querySelector(".rw-inspector-head h2"));
  }
  ...
  for (const theme of ["light", "dark"]) {
    ...
    const themed = await inspectVehicle(page);
  ...
  if (contextLoss) {
    ...
    await page.waitForFunction(() => document.querySelector(".rw-vehicle-canvas")?.dataset.lineInk === (document.documentElement.dataset.theme === "dark" ? "e0e0e0" : "252525") && document.querySelector(".rw")?.dataset.viewerStatus === "ready");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    for (const width of [320, 390, 1024, 1440]) {
      ...
      await checkVehicleControls({ page, base, artifacts, contextLoss: width === 1440 });
      assert.deepEqual(errors, []);
      await page.close();
      console.log(`${width}px: real vehicle geometry, paint, mesh, camera, touch, reduced motion and theme controls passed`);
    }
