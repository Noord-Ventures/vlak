...
import type { VlakCategory } from "@noorddev/vlak";
...
type ComponentLabel = { name: string; title: string; category: VlakCategory };
type NavIconGroup = { title: string };

function pageGroup(pathname: string, components: readonly ComponentLabel[], categories: readonly VlakCategory[]): VlakCategory | null {
  const match = pathname.match(/^\/components\/([^/]+)\/?$/);
  if (!match) return pathname.startsWith("/components") ? categories[0]! : null;
  return components.find((c) => c.name === match[1])?.category ?? categories[0]!;
}
...
function groupLinks(category: VlakCategory, components: readonly ComponentLabel[], iconGroups: readonly NavIconGroup[]) {
  if (category === "icons") {
...
function docsLabel(pathname: string, components: readonly ComponentLabel[]) {
  const page = docsPageForPath(pathname);
  if (page) return page.title;
  if (pathname.startsWith("/components/")) {
    const name = pathname.split("/")[2];
    return components.find((c) => c.name === name)?.title ?? "Components";
  }
  return "Components";
}
 ...
 */
export function DocsNavClient({ components, groups, icons }: {
  components: readonly ComponentLabel[];
  groups: readonly VlakCategory[];
  icons: readonly NavIconGroup[];
}) {
  const pathname = usePathname();
  const selected = pageGroup(pathname, components, groups);
  ...
  const docsLinks = (
    <>
      {docsNavigation.map(group => <div className="toc-mobile-doc-group" key={group.title}><p className="toc-mobile-label">{group.title}</p>{group.pages.map(page => <Link prefetch={false} key={page.href} href={page.href} className="toc-mobile-item" aria-current={here(pathname, page.href) ? "page" : undefined}>{page.title}</Link>)}</div>)}
    </>
  );
  ...
  const catalogMobile = (
    <>
      {groups.map((category) => {
        const items = groupLinks(category, components, icons);
      ...
      <>
        <div {...sx("toc-rail", navStyles.rail)}>
          <nav {...sx("toc toc-docs", navStyles.toc, navStyles.docs)} aria-label="Docs">
            {docsNavigation.map(group => <div className="toc-doc-group" key={group.title}><p {...sx("toc-label", navStyles.docsLabel)}>{group.title}</p>{group.pages.map(page => <Link prefetch={false} key={page.href} href={page.href} {...sx("toc-item", navStyles.item)} aria-current={here(pathname, page.href) ? "page" : undefined}>{page.title}</Link>)}</div>)}
          </nav>
        </div>
        <MobileToc label={docsLabel(pathname, components)}>{docsLinks}</MobileToc>
      </>
    );
  }

  const items = shown ? groupLinks(shown, components, icons) : [];
  ...
  return (
    <>
      ...
      <MobileToc label={docsLabel(pathname, components)}>{catalogMobile}</MobileToc>
    </>
  );
}
